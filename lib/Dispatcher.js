    ...
    _processFlows(options) {
        options
            ...
            .forEach(function (flow) {
                ...
                if (this.tags[flow.tag]) 
                    logger.warn('A flow with tag \'' + flow.tag + '\' already exists. Overriding...');
    ...
    _generateJob(tags, fileName) {
        ...
        for (let property in tags) {
            if (tags.hasOwnProperty(property)) {
                ...
                let runnablesOutput = [];
                let handlerRequire;
                let inputHookRequire;
                let outputHookRequire;
