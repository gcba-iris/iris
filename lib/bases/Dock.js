...
class Dock {
    ...
    validate(message) {
        ...
        if (this.config.maxMessageLength) {
            ...
            if (!validSize) {
                logger.verbose('Message has an invalid length: ' + message.length + ', max: ' + this.config.maxMessageLength);
        ...
        if (this._dispatcher.tags[tag]) {
            ...
            for (let i = 1; i < splitMessageLength; i += 2) {
                if (splitMessage[i].indexOf(this.config.parser.dataSeparator) === -1) 
                    validFormat = false;
                }
            
            if (!validFormat) {
                logger.verbose('Message has an invalid format: data separator \'' + this.config.parser.dataSeparator + '\' not found');
                this._invalidMessage(message);

                return false;
            }

            return {tag: tag, message: splitMessage};
        } else {
            logger.verbose('Message has an invalid tag: ' + tag);
            this._invalidMessage(message);
    ...
    parse(data, meta) {
        if (Array.isArray(data.message)) {
            ...
            for (let i = 0; i < length; i += 2) {
                result[data.message[i]] = data
                    .message[i + 1]
                    .split(this.config.parser.dataSeparator);
            }

            logger.verbose('Parsed message');

            return {tag: data.tag, meta: meta, data: result};
        } else {
            logger.warn('Unrecognized message body. Expected array');
    ...
    process(message, meta, callback) {
        ...
        if (typeof message === "string" || message instanceof String) 
            data = message;
        else if (message.toString && util.isFunction(message.toString)) 
            data = message.toString();
        else {
            logger.verbose('Received an unrecognized message type\nMessage metadata:\n' + JSON.stringify(meta, undefined, 4));
            this._invalidMessage(message);
        ...
        if (this._dispatcher) {
            ...
            if (validatedMessage) {
                ...
                if (parsedData) {
                    logger.verbose('Data sent to dispatcher');
                    parsedData.id = shortid.generate();
                    this
                        ._dispatcher
                        .dispatch(parsedData, callback);
                } else 
                    logger.error('No data returned by parser');
                }
            } else 
            logger.error('Dispatcher reference not set');
        }
    ...
    encode(response) {
        ...
        if (typeof response.message === "string" || response.message instanceof String) {
            logger.verbose('Encoded message');
            ...
            return response
                .message
                .join(this.config.encoder.dataSeparator);
        } else if (response.message === Object(response.message) && response.message.toString() === '[object Object]') {
            for (let property in response.message) {
                if (response.message.hasOwnProperty(property)) {
                    ...
                    } else if (value.toString && !util.isFunction(property)) 
            ...
            return result;
        } else if (!util.isFunction(response.message) && response.message.toString && util.isFunction(response.message.toString) && response.message.toString() !== '[object Object]') {
            logger.verbose('Encoded message using message own toString() method');

            return response
                .message
                .toString();
        } else {
            logger.error('Couldn\'t encode message');
    ...
    reply(response) {
        ...
        if (response.meta.dock === this._id) {
            ...
            logger.error('Invalid dock reference');
        }
    ...
    _invalidMessage(message) {
        ...
        this
            .logger
            .warn(`[${this.name.toUpperCase()} Dock] Invalid message: ${message}`);
    }
    ...
    _checkConfig(config) {
        ...
        process
            .stdout
            .write("\r\x1b[K");
        logger.verbose('Checked dock \'' + this.name + '\' config');
    }
