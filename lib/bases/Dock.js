...
class Dock {
        ...
        if (typeof response.message === "string" || response.message instanceof String) {
            ...
            for (let property in response.message) {
                if (response.message.hasOwnProperty(property)) {
                    ...
                    else if (!util.isFunction(property) && value.toString && util.isFunction(value.toString))
            ...
            this.logger.verbose('Encoded message using message own toString() method');
    ...
    _checkConfig(config) {
        ...
        this.logger.verbose('Checked dock \'' + this.name + '\' config');
    }
