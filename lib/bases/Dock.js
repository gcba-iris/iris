...
class Dock {
    ...
    validate(message) {
        ...
        if (this._dispatcher.tags[tag]) {
            ...
            for (let i = 1; i < splitMessageLength; i += 2) {
                if (!splitMessage[i].includes(this.config.parser.dataSeparator))
                    validFormat = false;
            }
    ...
    process(message, meta, callback) {
        ...
        if (this._dispatcher) {
            ...
            if (validatedMessage) {
                ...
                else logger.error('No data returned by parser');
            }
        }
        else logger.error('Dispatcher reference not set');
    }
    ...
    encode(response) {
        ...
        else if (!util.isFunction(response.message) && response.message.toString &&
            util.isFunction(response.message.toString) && response.message.toString() !== '[object Object]') {
    ...
    reply(response) {
        ...
        if (response.meta.dock === this._id) {
            if (this.send) {
                ...
                else logger.error('No message to send');
            }
        }
        else logger.error('Invalid dock reference');
    }
