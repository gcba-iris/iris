...
const newFolder = (folderName, spinner) => {
    if (checkFolder(folderName)) {
        handleErrors(['Directory \'' + folderName + '\' already exists'], spinner);
    }

    if (!createFolder(folderName)) {
        handleErrors(['Could not create directory \'' + folderName + '\''], spinner);
    }
};
...
const cli = {
     ...
     */
    version: () => {
        consoleLog.info(packageData.version);
    ...
    /**
     * Displays an Iris' project settings.
     *
     */
    config: () => {
        fail('This command is not implemented yet');
